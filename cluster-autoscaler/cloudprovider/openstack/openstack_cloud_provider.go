...
var _ cloudprovider.CloudProvider = (*OpenStackCloudProvider)(nil)
...
func (d *OpenStackCloudProvider) Name() string {
	return cloudprovider.OpenStackProviderName
}

// NodeGroups returns all node groups configured for this cloud provider.
func (d *OpenStackCloudProvider) NodeGroups() []cloudprovider.NodeGroup {
...
func (d *OpenStackCloudProvider) NodeGroupForNode(node *apiv1.Node) (cloudprovider.NodeGroup, error) {
...
func (d *OpenStackCloudProvider) HasInstance(node *apiv1.Node) (bool, error) {
	return true, cloudprovider.ErrNotImplemented
}

// Pricing returns pricing model for this cloud provider or error if not
// available. Implementation optional.
func (d *OpenStackCloudProvider) Pricing() (cloudprovider.PricingModel, errors.AutoscalerError) {
...
func (d *OpenStackCloudProvider) GetAvailableMachineTypes() ([]string, error) {
...
func (d *OpenStackCloudProvider) NewNodeGroup(
	machineType string,
...
func (d *OpenStackCloudProvider) GetResourceLimiter() (*cloudprovider.ResourceLimiter, error) {
	return d.resourceLimiter, nil
}

// GPULabel returns the label added to nodes with GPU resource.
func (d *OpenStackCloudProvider) GPULabel() string {
	return GPULabel
}

// GetAvailableGPUTypes return all available GPU types cloud provider supports.
func (d *OpenStackCloudProvider) GetAvailableGPUTypes() map[string]struct{} {
	return nil
}

// GetNodeGpuConfig returns the label, type and resource name for the GPU added to node. If node doesn't have
// any GPUs, it returns nil.
func (d *OpenStackCloudProvider) GetNodeGpuConfig(node *apiv1.Node) *cloudprovider.GpuConfig {
	return gpu.GetNodeGPUFromCloudProvider(d, node)
}

// Cleanup cleans up open resources before the cloud provider is destroyed,
// i.e. go routines etc.
func (d *OpenStackCloudProvider) Cleanup() error {
	return nil
}
...
func (d *OpenStackCloudProvider) Refresh() error {
...
// BuildOpenStack builds the OpenStack cloud provider.
func BuildOpenStack(_ config.AutoscalingOptions, do cloudprovider.NodeGroupDiscoveryOptions, rl *cloudprovider.ResourceLimiter) cloudprovider.CloudProvider {
	manager, err := newManager()
	if err != nil {
		klog.Fatalf("Failed to create OpenStack manager: %v", err)
	}

	provider, err := newOpenStackCloudProvider(manager, rl)
	if err != nil {
	...
	for _, nodegroupSpec := range do.NodeGroupSpecs {
		...
		manager.nodeGroups[spec.name] = &openstackNodeGroup{
			manager:            manager,
			id:                 spec.name,
...
func createNodePoolSpec(groupSpec string) (*openstackNodeGroupSpec, error) {
	...
	definition := openstackNodeGroupSpec{
...
func newOpenStackCloudProvider(manager *openstackManager, rl *cloudprovider.ResourceLimiter) (*OpenStackCloudProvider, error) {
