...
// openstackNodeGroup implements cloudprovider.NodeGroup interface. openstackNodeGroup contains
...
type openstackNodeGroupSpec struct {
	name         string
	minSize      int
	maxSize      int
	region       string
...
func (n *openstackNodeGroup) MaxSize() int {
	return n.maxSize
}

// MinSize returns minimum size of the node group.
func (n *openstackNodeGroup) MinSize() int {
	return n.minSize
}

// GetOptions returns NodeGroupAutoscalingOptions that should be used for this particular
// NodeGroup. Returning a nil will result in using default options.
func (n *openstackNodeGroup) GetOptions(defaults config.NodeGroupAutoscalingOptions) (*config.NodeGroupAutoscalingOptions, error) {
...
// required.
func (n *openstackNodeGroup) TargetSize() (int, error) {
...
func (n *openstackNodeGroup) IncreaseSize(delta int) error {
	if delta <= 0 {
	...
	// There is no "Server Group" in OpenStack Cloud, we need to create every
...
func (n *openstackNodeGroup) AtomicIncreaseSize(delta int) error {
...
func (n *openstackNodeGroup) DeleteNodes(nodes []*apiv1.Node) error {
	n.clusterUpdateMutex.Lock()
	defer n.clusterUpdateMutex.Unlock()
...
func (n *openstackNodeGroup) DecreaseTargetSize(delta int) error {
	n.targetSize = n.targetSize + delta
	return nil
}

// Id returns an unique identifier of the node group.
func (n *openstackNodeGroup) Id() string {
	return n.id
}

// Debug returns a string containing all information regarding this node group.
func (n *openstackNodeGroup) Debug() string {
...
func (n *openstackNodeGroup) Nodes() ([]cloudprovider.Instance, error) {
...
func (n *openstackNodeGroup) TemplateNodeInfo() (*schedulerframework.NodeInfo, error) {
...
// required.
func (n *openstackNodeGroup) Exist() bool {
...
// optional.
func (n *openstackNodeGroup) Create() (cloudprovider.NodeGroup, error) {
...
func (n *openstackNodeGroup) Delete() error {
...
func (n *openstackNodeGroup) Autoprovisioned() bool {
...
func newNodeName(n *openstackNodeGroup) string {
	return fmt.Sprintf("%s-%x", n.id, rand.Int63())
}

func buildNodeGroupLabels(n *openstackNodeGroup) (map[string]string, error) {
	...
	labels := map[string]string{
		...
		"csi.openstack.cloud/location": n.region,
		nodeGroupLabel:                 n.id,
	}
...
func getMachineTypeResourceList(m *openstackManager, instanceType string) (apiv1.ResourceList, error) {
...
func serverTypeAvailable(manager *openstackManager, instanceType string, region string) (bool, error) {
...
func instanceTypeArch(manager *openstackManager, instanceType string) (string, error) {
	serverType, err := manager.cachedServerType.getServerType(instanceType)
	if err != nil {
		return "", err
	}
...
func createServer(n *openstackNodeGroup) error {
...
// server.
func findImage(n *openstackNodeGroup, serverType *hcloud.ServerType) (*hcloud.Image, error) {
...
func (n *openstackNodeGroup) resetTargetSize(expectedDelta int) {
