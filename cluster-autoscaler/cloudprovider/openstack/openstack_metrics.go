...
func instrumentedRoundTripper() http.RoundTripper {
	inFlightRequestsGauge := k8smetrics.NewGauge(&k8smetrics.GaugeOpts{
		Name: fmt.Sprintf("openstack_%s_in_flight_requests", subsystemIdentifier),
		Help: fmt.Sprintf("A gauge of in-flight requests to the openstack %s.", subsystemIdentifier),
	})

	requestsPerEndpointCounter := k8smetrics.NewCounterVec(
		&k8smetrics.CounterOpts{
			Name: fmt.Sprintf("openstack_%s_requests_total", subsystemIdentifier),
			Help: fmt.Sprintf("A counter for requests to the openstack %s per endpoint.", subsystemIdentifier),
		},
	...
	requestLatencyHistogram := k8smetrics.NewHistogramVec(
		&k8smetrics.HistogramOpts{
			Name:    fmt.Sprintf("openstack_%s_request_duration_seconds", subsystemIdentifier),
			Help:    fmt.Sprintf("A histogram of request latencies to the openstack %s .", subsystemIdentifier),
